...
- **Controls Bar UX Polish (2026-01-25)**: Improved consistency and usability of header controls
  - Standardized checkbox sizing across control groups
  - Added consistent focus rings for keyboard navigation and clearer hover affordances
  - Added info tooltips (with indicator icon) for advanced toggles (e.g., Cognitive Load, Platform Consumers, Flow Metrics)
  - Added explicit `:focus-visible` styling on checkbox/radio inputs
  - Normalized icon button sizing for the keyboard shortcuts button
...
- **Local AI context refresh (2026-01-24)**: Updated `.ai-context.md` to match current run workflow
  - Documented canonical local server start command on Windows using venv Python
  - Added `TT_TEAMS_VARIANT` instructions for switching TT datasets (`tt-teams` vs `tt-teams-initial`)
  - Fixed outdated `run-all-tests.ps1` path references to `scripts/run-all-tests.ps1`
...
- **Pre-TT Team Template (2026-01-06)**: Created comprehensive template for current organizational state
  ...
  - Renamed existing templates: `tt-team-api-template-base.md` and `tt-team-api-template-extended.md`
