---
name: Customer Solutions Department
type: department
line_manager: Marcus Thompson
reports_to: VP Professional Services
metadata:
  department: Customer Solutions
  size: 27
  budget_type: profit-center
  billable: true
---

# Customer Solutions Department

Professional services team responsible for customer-specific implementations, customizations, and regional adaptations of LogiCore Platform. Operates as a billable profit center.

## Department Goals
- 💰 Maximize billable hours and revenue
- 💰 Deliver customer requirements quickly
- 💰 High utilization rates (90%+ target)
- 💰 Customer satisfaction and renewals
- 💰 Expand into new regions

## Priorities
- Customer deadlines above all else
- "Whatever the customer wants, they get"
- Fast delivery over maintainability
- Regional compliance and customization
- Invoice as many hours as possible

## Teams (5 regional teams, 27 people)
- Americas E-commerce Solutions Team (5)
- Americas Supply Chain Solutions Team (7)
- European Logistics Solutions Team (6)
- European Retail Solutions Team (5)
- European Transport Solutions Team (4)

## Operating Model
- Billable hourly work model
- Customer-specific code branches and customizations
- Regional regulatory compliance implementations
- Direct customer engagement and support

## Typical Conflicts With Other Departments
- **vs Engineering**: "We don't have time to use your APIs, we need to deliver NOW!"
- **vs Product Management**: "Our customers need features that aren't on your roadmap"
- **vs Architecture**: "We can't wait for your standards, customers are paying us"
